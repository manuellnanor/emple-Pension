...
export default function BranchFinder() {
  ...
  return (
    <div id="branch-finder-component" className="w-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50 p-4 border border-[#f1f1f1]">
        {/* Buttons filters */}
          ...
          {filteredBranches.length > 0 ? (
            filteredBranches.map((branch, index) => (
              <motion.div
                ...
                className="group bg-white p-6 border border-[#f1f1f1] shadow-xs hover:bg-[#052e16] hover:border-transparent hover:shadow-md transition-all duration-300 flex flex-col justify-between [&:hover_h4]:!text-white"
              >
                <div>
                  ...
                  <h4 className="font-sans text-[15px] font-bold text-gray-900 group-hover:!text-white mb-2 transition-colors duration-300">{branch.name}</h4>
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="col-span-2 text-center py-12 bg-gray-50 border border-dashed border-[#f1f1f1]"
            >
